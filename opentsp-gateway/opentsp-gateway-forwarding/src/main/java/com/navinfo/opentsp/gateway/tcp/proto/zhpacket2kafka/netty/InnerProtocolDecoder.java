...
import com.navinfo.opentsp.gateway.tcp.proto.zhpacket2kafka.kit.cache.DataPacketCache;
...
public class InnerProtocolDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        if (in.writerIndex() > in.readerIndex()) {
            ...
            in.readBytes(bytes);
            log.error("HY-->RP:" + ctx.channel().toString() + "收到终端原始数据:" + Convert.bytesToHexString(bytes));
            if (startPacker != null) {
                //有分包拼接上一包尾数据
