...
public class MMMutualDecoder implements ProtocolDecoder {
    private static final Logger logger = LoggerFactory.getLogger(MMMutualDecoder.class);

    @Override
    public void decode(IoSession session, IoBuffer in, ProtocolDecoderOutput out)
            throws Exception {
//		logger.info("[DA<MM]: " + in.getHexDump());
        int limit = in.limit();
        byte[] currentBytes = new byte[limit];
        in.get(currentBytes, 0, limit);
        MMMutualSession mutualSession = MMMutualSessionManage.getInstance().getMutualSession(session.getId());
    ...
    private void build(byte[] packet, IoSession session,
        ...
        if ((cmdProperty & 8192) > 0) {
            ...
            int serial = Convert.byte2Int(ArraysUtils.subarrays(bytes, 15, 2), 2);
            ;// 消息包封装项,包序号
    ...
    private boolean sessionCheck(String unqieMark, IoSession session) {
        String key = String.valueOf(session.getAttribute("uniqueMark"));
