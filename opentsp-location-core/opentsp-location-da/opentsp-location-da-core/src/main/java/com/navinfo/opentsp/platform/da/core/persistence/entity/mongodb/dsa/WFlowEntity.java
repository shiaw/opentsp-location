...
public class WFlowEntity  extends BaseMongoEntity implements Serializable{
	private long _id;
	...
	public   void dbObjectToBean(DBObject dbObjectDetail){
		...
		if(dataList!=null&&dataList.size()>0){
			this.dataList=new ArrayList<DAWFlow>();
			for (DBObject d : dataList) {
				DAWFlow wFlow = new DAWFlow();
				wFlow.dbObjectToBean(d);
				this.dataList.add(wFlow);
			}
