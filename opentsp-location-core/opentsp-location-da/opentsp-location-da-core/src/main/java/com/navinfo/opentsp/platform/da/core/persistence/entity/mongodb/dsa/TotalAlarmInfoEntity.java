...
@LCTable(name="TotalAlarmInfo")
public class TotalAlarmInfoEntity  extends BaseMongoEntity implements Serializable{
	private long _id;
	private long terminal_id;
	private long day;
	private List<DATotalAlarmInfo> dataList;
	public   void dbObjectToBean(DBObject dbObjectDetail){
		...
		if(dataList!=null&&dataList.size()>0){
			this.dataList=new ArrayList<DATotalAlarmInfo>();
			for (DBObject d : dataList) {
				DATotalAlarmInfo dATotalAlarmInfo = new DATotalAlarmInfo();
				dATotalAlarmInfo.dbObjectToBean(d);
				this.dataList.add(dATotalAlarmInfo);
			}
	...
	public List<DBObject> toBatchDBObject(){
		...
		if(this.dataList!=null&&this.dataList.size()!=0){
			for (DATotalAlarmInfo d : this.dataList) {
	...
	public List<DATotalAlarmInfo> getDataList() {
		return dataList;
	}
	public void setDataList(List<DATotalAlarmInfo> dataList) {
