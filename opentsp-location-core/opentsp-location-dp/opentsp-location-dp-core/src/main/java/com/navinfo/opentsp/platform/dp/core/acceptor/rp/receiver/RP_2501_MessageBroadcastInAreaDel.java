...
import com.navinfo.opentsp.platform.location.protocol.terminal.setting.LCMessageBroadcastInAreaDel.MessageBroadcastInAreaDel;
...
@RPAnno(id="2501")
public class RP_2501_MessageBroadcastInAreaDel extends RPCommand {
	...
	public int processor(Packet packet) {
		try {
			//本地删除缓存
			MessageBroadcastInAreaDel messageBroadcastInAreaDel = MessageBroadcastInAreaDel.parseFrom(packet.getContent());
			
			List<Long> areaIds = messageBroadcastInAreaDel.getAreaIdentifysList();
			
			for(Long areaId : areaIds){
				//删除的是多个个区域
//				AreaCommonCache.getInstance().removeAreaEntity(areaId);
//				RuleCommonCache.getInstance().removeRuleEntityByAreaId(170080, areaId);
				areaCommonCache.removeAreaEntity(areaId);
				ruleCommonCache.removeRuleEntityByAreaId(170080, areaId);

				//构建DA数据
				if(messageBroadcastInAreaDel.getSaveSign()){
//					MessageBroadcastInAreaDel.Builder mbdelBuilder = MessageBroadcastInAreaDel.newBuilder();
//					mbdelBuilder.addAreaIdentifys(areaId);
//					mbdelBuilder.setSaveSign(messageBroadcastInAreaDel.getSaveSign());
					...
					delBuilder.setRegularCode(LCRegularCode.RegularCode.messageBroadcast);
