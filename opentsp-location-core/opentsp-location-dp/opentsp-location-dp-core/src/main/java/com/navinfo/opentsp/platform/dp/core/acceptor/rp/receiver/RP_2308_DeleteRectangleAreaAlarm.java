...
@RPAnno(id="2308")
public class RP_2308_DeleteRectangleAreaAlarm extends RPCommand {
	...
	public int processor(Packet packet) {
		try {
			...
			log.info("[DP: RectangleAreaAlarm] 矩形区域规则Delete操作. 删除缓存与AreaCache中的矩形区域对象， RuleCache中对应的规则对象.");
			//缓存删除
			DeleteRectangleAreaAlarm draa =DeleteRectangleAreaAlarm.parseFrom(packet.getContent());
			long terminalId = Convert.uniqueMarkToLong(packet.getUniqueMark());
			List<Long> list = draa.getAreaIdentifyList();
			for(long areaId : list) {
//				AreaCache.getInstance().removeAreaEntity(terminalId, areaId, AreaType.rectangle);
//				RuleCache.getInstance().getRuleEntity(terminalId).removeRuleForArea(areaId, AreaType.rectangle);
			...
			//
			log.info("[DP: RectangleAreaAlarm] 矩形区域规则Delete操作. 给终端发送通用应答 [0x3001]");
			super.commonResponses(4,packet.getFrom(), packet.getSerialNumber(), packet.getCommand(), PlatformResponseResult.success_VALUE);
			
			
			log.info("[DP: RectangleAreaAlarm] 矩形区域规则Delete操作. 给DA发送Dlete消息 [0x0931]");
			//构建DA数据
